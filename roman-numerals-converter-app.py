...
def convert_html():
    if request.method=="POST":
        ...
        if int(number1)<1 or not number1.isdigit() or int(number1)>3999:
            return render_template("convert.html", developer_name='Seyma')
        else:
            return render_template("convert.html", developer_name='Seyma', convert=convert(number1),number=number1)
    else:
